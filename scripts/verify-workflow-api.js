...
const ASSISTANTE = { matricule: 'PCRS 0038', role: 'AssistanteLogistique' };
...
const DIRECTRICE = { matricule: 'DPR', role: 'Directrice' }; // Using DPR as found in DB

// Native fetch is available in Node 18+

async function callApi(method, path, user, body) {
    const headers = {
...
async function run() {
    console.log('--- STARTING WORKFLOW VERIFICATION (JS) ---');
    ...
    const createRes = await callApi('POST', '/demandeur', REQUESTER, {
        ...
        items: [{
            type: 'NAV-ACH',
            objet: 'Test Workflow Automated JS',
            motif: 'Verification',
            qte: 1,
            puHt: 100000,
            montant: 100000,
            description: 'Test Item JS'
        }]
    });
    ...
    if (v7.data.data.action === 'validee_finale') {
        console.log(`✅ Step 7 Validated! Status: VALIDEE`);
